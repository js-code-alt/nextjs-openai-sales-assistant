...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const [insertResult] = await pool.execute(
      'INSERT INTO legal_documents (name, description) VALUES (?, ?)',
      [name, content.substring(0, 500)] // Store first 500 chars as description
    )
    const insertResultArray = insertResult as { insertId: number }
    const legalId = insertResultArray.insertId

    // Split content into sections
    // For legal documents, try to split by section numbers/headers first
    let sections: string[] = []
    ...
    console.log(`[Legal Document: ${name}] Creating ${sections.length} sections with embeddings`)
    ...
    for (const [index, section] of sections.entries()) {
      try {
        ...
        await pool.execute(
          'INSERT INTO legal_sections (legal_id, content, section_title, token_count, embedding) VALUES (?, ?, ?, ?, Vec_FromText(?))',
          [
            legalId,
            section,
            sectionTitle,
    ...
    return res.status(200).json({
      ...
      message: 'Legal document uploaded and embeddings generated successfully',
    })
  } catch (err: unknown) {
