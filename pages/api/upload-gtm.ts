...
async function parseForm(req: NextApiRequest): Promise<{ fields: formidable.Fields; files: formidable.Files }> {
  return new Promise((resolve, reject) => {
    const form = formidable({
      ...
      // Our document name field is small, so we don't need this limit
    })
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const [insertResult] = await pool.execute(
      'INSERT INTO gtm_documents (name, description) VALUES (?, ?)',
      [name, content.substring(0, 500)] // Store first 500 chars as description
    )
    const insertResultArray = insertResult as { insertId: number }
    const gtmId = insertResultArray.insertId

    // Split content into sections
    // For GTM documents, try to split by section numbers/headers first, similar to legal
    let sections: string[] = []
    
    // Try to split by section numbers (e.g., "# 1. ", "1.1. ", "2.1. ", "# 2. ")
    const sectionNumberPattern = /(?:^|\n)#?\s*\d+\.\s*\d*\.?\s+/gm
    const matches = [...content.matchAll(sectionNumberPattern)]
    ...
    console.log(`[GTM Document: ${name}] Creating ${sections.length} sections with embeddings`)
    ...
    for (const [index, section] of sections.entries()) {
      try {
        ...
        // Extract a title from the section - look for section numbers in first few lines
        const firstLines = section.split('\n').slice(0, 3).join(' ')
        const sectionNumberMatch = firstLines.match(/#?\s*(\d+\.\s*\d*\.?\s*.{0,100})/)
        const sectionTitle = sectionNumberMatch 
          ? sectionNumberMatch[1].trim().substring(0, 500)
          : section.split('\n')[0].trim().substring(0, 500) || `Section ${index + 1}`

        // Convert embedding array to MariaDB VECTOR format string
        const embeddingVectorString = arrayToVectorString(embedding)

        await pool.execute(
          'INSERT INTO gtm_sections (gtm_id, content, section_title, token_count, embedding) VALUES (?, ?, ?, ?, Vec_FromText(?))',
          [
            gtmId,
            section,
            sectionTitle,
    ...
    return res.status(200).json({
      ...
      message: 'GTM document uploaded and embeddings generated successfully',
    })
  } catch (err: unknown) {
