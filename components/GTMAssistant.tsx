...
export function GTMAssistant() {
  ...
  React.useEffect(() => {
    if (!hasFetchedInitialSuggestions.current && chatHistory.length === 0 && !isLoadingSuggestions) {
      ...
      fetch('/api/suggested-questions?type=gtm')
        .then((res) => res.json())
        .then((data) => {
          if (data.questions && Array.isArray(data.questions) && data.questions.length > 0) {
            setSuggestedQuestions(data.questions)
          } else {
            // No GTM documents uploaded, set empty array to show message
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 min-h-[600px] flex flex-col">
        <div className="mb-4 pb-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
            Ask about Go-to-Market Positioning
          </h2>
        ...
        <div className="flex-1 overflow-y-auto mb-4 space-y-6">
          {allHistory.length === 0 && !isLoading && (
            <div className="text-center text-gray-500 dark:text-gray-400 py-8">
              ...
              <p className="mb-6">Start by asking a question about competitive positioning and go-to-market strategies...</p>
              
              {/* Show message if no GTM documents uploaded */}
              {!isLoadingSuggestions && suggestedQuestions.length === 0 && (
                <div className="max-w-3xl mx-auto mt-4">
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    Upload GTM documents in Settings to generate suggested questions
                  </p>
                </div>
              )}
        ...
        <form onSubmit={handleSubmit} className="mt-auto">
          <div className="relative">
            <Input
              placeholder="Ask a question about competitive positioning and go-to-market strategies..."
              name="search"
              value={query}
